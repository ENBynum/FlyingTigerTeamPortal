...
import {
	AspectRatio, Burger, Center, Container, Grid, Group, Image, Menu, Stack, Title
} from '@mantine/core'
import { notifications } from '@mantine/notifications'

import Authenticate from '../../services/get.authenticate'
    ...
    return <>
        <Group w={'100%'} h={'100%'} justify='start' align='center'>
            <Menu shadow='xl'>
                ...
                <Menu.Dropdown>
                    {!location.startsWith(routes.soldierDashboard.replace(':dodid', '')) && <Menu.Item onClick={() => navigate(routes.soldierDashboard.replace(':dodid', auth.dodid as string))}>
                        Dashboard
                    </Menu.Item>}
                    {!location.startsWith(routes.soldierRSTSubmit.replace(':dodid', '')) && <Menu.Item onClick={() => navigate(routes.soldierRSTSubmit.replace(':dodid', auth.dodid as string))}>
                        Submit RST Request
                    </Menu.Item>}
                </Menu.Dropdown>
            </Menu>
